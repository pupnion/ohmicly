...
async function getStats() {
  try {
    ...
    const [toolsRes, blogRes, faqRes] = await Promise.all([
      supabase.from("tools").select("id, is_popular"),
      supabase.from("blog_posts").select("id, published, title, created_at").order("created_at", { ascending: false }),
      supabase.from("faqs").select("id"),
    ]);

    // These tables may not exist yet, so handle gracefully
    ...
    const unpublishedBlogs = blogs.filter((b) => !b.published).length;

    return {
      toolCount: tools.length,
      popularToolCount: tools.filter((t) => t.is_popular).length,
...
export default async function DashboardPage() {
  ...
  const cards = [
    ...
    },
    {
      ...
      value: stats.popularToolCount,
      icon: Star,
      color: "text-green-600",
      bg: "bg-green-100",
    },
    {
      label: "মোট Blog Posts",
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {cards.map((card) => (
          ...
          >
            ...
            <p className="text-3xl font-bold text-brand-navy font-en">
              {card.value}
            </p>
            <p className="text-sm text-slate-500 font-bn mt-1">
              {card.label}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-100">
          <h2 className="text-lg font-bold text-slate-800 font-bn mb-4">
            Recent Downloads
          </h2>
          ...
          {stats.recentBlogs.length > 0 ? (
            <div className="space-y-3">
              {stats.recentBlogs.map((post) => (
                ...
                >
                  <div>
                    <span className="text-sm text-slate-700 font-bn">
                      {post.title}
                    </span>
                    {!post.published && (
                      <span className="ml-2 text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded font-bn">
                        Draft
                      </span>
                    )}
                  </div>
